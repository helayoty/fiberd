...
//
// Build:  go test -c -o bin/grant-conform ./cmd/grant-conform
...
func main() {
	fmt.Fprintln(os.Stderr, "grant-conform is a test binary: build it with `go test -c -o bin/grant-conform ./cmd/grant-conform` and run that.")
	os.Exit(2)
}
